...
export async function sendInviteEmail(opts: {
  to: string;
  inviteUrl: string;
  inviterLabel?: string; // e.g. username/display name if you have it
}) {
  const fromInvites = requireEnv("EMAIL_FROM_INVITES");

  const inviterLine = opts.inviterLabel
    ? `
      <tr>
        <td style="padding: 10px 18px 0;">
          <div style="
            ...
            line-height: 1.6;
          ">
            Invited by <b style="color: rgba(255,255,255,0.92)">${escapeHtml(
              opts.inviterLabel
            )}</b>.
          </div>
        </td>
  ...
  return resend.emails.send({
    ...
    html: renderBrandedEmail({
      ...
      preheader: "Accept your invite to join BlockPools.",
      intro:
        "Use the button below to accept your invite. Once you log in, we’ll attribute the signup to the inviter.",
